...
@section('title', 'Edit Role with Permissions & Users')
...
<div class="container-xxl">
  <div class="card">
    ...
    <h5 class="card-header text-center fw-bold fs-5 d-flex align-items-center justify-content-center"
      ...
      <i class="bx bx-shield fs-4"></i>{{__('fieldsName.edit')}} {{ $role->name }} {{__('fieldsName.role')}}
    </h5>
    ...
    <div class="card-body">
      ...
      <form action="{{ route('update.roles', $role) }}" method="POST">
        ...
        <div class="mb-4">
          ...
          <input type="text" id="role_name" name="name" class="form-control" value="{{ old('name', $role->name) }}"
                ...
                <tr>
                  <td>
                    <input type="checkbox" name="users[]" value="{{ $admin->id }}"
                      {{ $role->users->contains($admin->user) ? 'checked' : '' }}>
                  </td>
        ...
        <button type="submit" class="btn btn-primary">
          <i class="bx bx-refresh"></i> {{__('fieldsName.updaterole')}}
        </button>
